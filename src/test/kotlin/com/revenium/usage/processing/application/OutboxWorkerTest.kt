...
class OutboxWorkerTest {
    ...
    private val messages = mockk<OutboxMessageRepository>(relaxed = true)
    ...
    fun `does nothing when the queue is empty`() {
        every { claims.claimBatch(any(), any()) } returns emptyList()
    ...
    fun `processes every message in the claimed batch`() {
        every { claims.claimBatch(any(), any()) } returns listOf(work(rawEventId = 1L), work(rawEventId = 2L))
    ...
    fun `rates each message under the tenant on its own row`() {
        ...
        every { claims.claimBatch(any(), any()) } returns
    ...
    fun `leaves no tenant in scope after the cycle`() {
        every { claims.claimBatch(any(), any()) } returns listOf(work())
        every { ratingService.rate(any()) } returns ratedOutcome()
