...
class OutboxMessageTest {
    ...
    private fun message() = OutboxMessage(tenantId = TenantId("tenant-a"), rawEventId = 1L)
    ...
    fun `marking done clears the last error`() {
        val message = message()
            .markFailed(now, "transient failure", maxAttempts = 5, backoffBase = backoff)
            .markDone(now)
    ...
    fun `a failure schedules a retry and counts the attempt`() {
        val message = message().markFailed(now, "boom", maxAttempts = 5, backoffBase = backoff)
    ...
    fun `backoff doubles with each attempt`() {
        ...
        val first = message().markFailed(now, "boom", maxAttempts = 10, backoffBase = backoff)
        assertEquals(now.plusSeconds(10), first.nextAttemptAt)

        val second = first.markFailed(now, "boom", maxAttempts = 10, backoffBase = backoff)
        assertEquals(now.plusSeconds(20), second.nextAttemptAt)

        val third = second.markFailed(now, "boom", maxAttempts = 10, backoffBase = backoff)
        assertEquals(now.plusSeconds(40), third.nextAttemptAt)
    }

    @Test
    fun `dead-letters once the attempt budget is exhausted`() {
        val message = (1..3).fold(message()) { m, _ ->
            m.markFailed(now, "boom", maxAttempts = 3, backoffBase = backoff)
        }
    ...
    fun `retries right up to the final attempt`() {
        ...
        val first = message().markFailed(now, "boom", maxAttempts = 3, backoffBase = backoff)
        assertEquals(OutboxStatus.PENDING, first.status)

        val second = first.markFailed(now, "boom", maxAttempts = 3, backoffBase = backoff)
        assertEquals(OutboxStatus.PENDING, second.status)

        val third = second.markFailed(now, "boom", maxAttempts = 3, backoffBase = backoff)
        assertEquals(OutboxStatus.FAILED, third.status)
    }

    @Test
    fun `truncates an oversized error rather than failing to record it`() {
        val message = message().markFailed(now, "x".repeat(5000), maxAttempts = 5, backoffBase = backoff)
    ...
    fun `a missing pricing rule does not count against the attempt budget`() {
        ...
        val message = (1..10).fold(message()) { m, _ -> m.markUnrated(now, Duration.ofMinutes(5)) }
    ...
    fun `an unrated message retries on its own slower cadence`() {
        val message = message().markUnrated(now, Duration.ofMinutes(5))

        assertEquals(now.plus(Duration.ofMinutes(5)), message.nextAttemptAt)
        assertTrue(message.lastError.orEmpty().contains("pricing rule"))
    }
    ...
    fun `quarantine holds a message without billing or discarding it`() {
        val message = message().markQuarantined(now, "occurredAt is 400 days old")
    ...
    fun `every transition stamps updatedAt`() {
        ...
        assertEquals(later, message().markDone(later).updatedAt)
        assertEquals(later, message().markFailed(later, "e", 5, backoff).updatedAt)
        assertEquals(later, message().markUnrated(later, backoff).updatedAt)
        assertEquals(later, message().markQuarantined(later, "r").updatedAt)
    }
}
