...
@Repository
interface PricingRuleJpaRepository : JpaRepository<PricingRule, Long> {
    ...
    fun findByTenantIdOrderByTransactionCodeAscEffectiveFromAsc(tenantId: String): List<PricingRule>
}

/**
 * The JPA adapter for [PricingRuleLookup].
 *
 * Deliberately thin: it translates typed domain values to the repository's primitives
 * and nothing else. Any logic here would be logic the rating unit tests cannot see.
 */
@Repository
class JpaPricingRuleLookup(
    ...
    override fun findApplicable(
        ...
        occurredAt: Instant,
    ): PricingRule? = repository.findApplicable(tenant.value, code.value, occurredAt)
}
