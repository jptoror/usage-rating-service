package com.revenium.usage.pricing.domain.port.out
...
interface PricingRuleLookup {
    ...
    /** The tenant's rules, ordered so two reads of the same data read alike. */
    fun findAllFor(tenant: TenantId): List<PricingRule>

    /** One rule, scoped to the tenant so a wrong id is a miss rather than a leak. */
    fun findById(tenant: TenantId, id: Long): PricingRule?
}
